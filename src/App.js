...
submitAnswer(){
  ...
  render() {
    ...
    const results_component = (
      <Results score={this.state.score} end_message="Congratulation, you have finished!" />
    );
    return (
      <div>
        <h2>Quiz App</h2>
          {this.state.progress >= this.quiz_data.length ? results_component : in_quiz}
      </div>
    );
  }
}
